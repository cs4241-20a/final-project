...
const addUserToRandomLobby = () => {
  ...
  lobbyKeys.forEach((key) => {
    if (lobbies[key]) {
      ...
      if (Object.keys(lobbies[key].clients.length) < 8) {
        //Only once please
        if (!lobbyId) {
          lobbyId = key;
        }
...
//   // carrier
// ) => {
//   const getLetterIndex = (letter) => columnLetters.indexOf(letter);
...
//         for (let i = 0; i < length; i++) {
//           posArray.push({
//             position: `${columnLetters[baseLetterIndex + i]}${start.charAt(1)}`,
//             hit: false,
//           });
//         }
//       } else {
...
io.on("connect", (socket) => {
  ...
  socket.on("joinNextLobby", () => {
    ...
    if (newLobbyId) {
      ...
      // }
      //}
      lobbies[newLobbyId].clients[socket.id] = {
        name: `Jeff${Date()}`,
      };
      ...
      lobbies[generatedLobbyid].clients[socket.id] = {
        name: `Jeff${Date()}`,
      };
  ...
  socket.on("joinSpecificLobby", (lobbyCode) => {
    ...
    if (!!desiredLobby) {
      if (Object.keys(lobbies[lobbyCode].clients).length < 8) {
        //Theres room
        desiredLobby[lobbyCode].clients[socket.id] = {
          name: `Jeff${Date()}`,
        };
      } else {
        //No Room
  ...
  socket.on("leaveLobby", () => {
    ...
    if (Object.keys(lobbies[lobbyId].clients).length <= 0) {
  ...
  socket.on("createLobby", (lobbyCode) => { 
      ...
      lobbies[lobbyCode].clients[socket.id] = {
        name: `Jeff${Date()}`,
      };
      lobbyId = lobbyCode;
      socket.send(`You Joined ${lobbies[lobbyCode]}`)
    })
